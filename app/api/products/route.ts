import { GetIdsProductUseCase } from '@/backend/products/applications/usecases/GetIdsProductUseCase';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const products = await new GetIdsProductUseCase(productRepository).findManyByIds(ids);
...
export async function POST(req: NextRequest) {
  try {
    ...
    // 1) 파싱/검증
    const parsed: number[] = ids.map((v: number) => Number(v)).filter((n: number) => Number.isInteger(n) && n > 0);

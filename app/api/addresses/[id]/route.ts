...
export async function DELETE(req: Request | NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    const session = await getServerSession(authOptions);
    if (!session?.user?.id) {
      return NextResponse.json({ message: '로그인이 필요합니다.' }, { status: 401 });
    ...
    if (result.code === 'P2003') {
      return NextResponse.json({ message: '결제중인 주소가 있습니다.' });
    }
    ...
    return NextResponse.json({ message: (error as Error).message || '주소 삭제 중 오류 발생' }, { status: 500 });
  }
}

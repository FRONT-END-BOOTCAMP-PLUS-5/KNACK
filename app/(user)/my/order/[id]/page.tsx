    ...
    (async () => {
      try {
        ...
        const mapped: ReceiptItem[] = (data.orders ?? []).map((o: OrderResponse) => ({
          ...
          title: o.product?.engName ?? o.productTitle ?? o.title ?? '',
          optionText: [o.size ?? o.option ?? o.variant, o.shippingType ?? o.deliveryType ?? '일반배송']
            .filter(Boolean)
            .join(' / '),
          status: o.statusText ?? o.status,
          imageUrl: o.product?.thumbnailImage ?? o.thumbnailImage ?? o.imageUrl ?? '/placeholder.png',
        }));
        ...
        const merged: PaymentData = {
          ...
          cardMasked:
            data.cardMasked ?? data.card?.numberMasked ?? data.cardInfo?.masked ?? 'KB국민카드 ••••••••••700*',
        };

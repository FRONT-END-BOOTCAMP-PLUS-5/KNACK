...
import { IRecommendProdcut } from '@/types/product';
...
const Recommends = () => {
  ...
  const [recommends, setRecommends] = useState<IRecommendProdcut[]>([]);
  ...
  return (
    <Flex className={styles.container} paddingHorizontal={14} paddingVertical={30} direction="column">
      ...
      <Flex className={styles.recommend_box} gap={10} paddingVertical={20}>
        {recommends?.map((item) => (
          <Flex key={`recommend_prodcuts_${item?.id}`} direction="column">

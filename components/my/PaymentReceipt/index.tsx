...
export default function PaymentReceipt({ items, totals, info }: { items: Item[]; totals: Totals; info: Info }) {
      ...
      <div className={styles.page}>
        ...
        <section className={styles.card}>
          {items.map((item) => (
            <article key={item.id} className={styles.item_row} onClick={() => router.push(`/my/buying/${item.id}`)}>
              <Image
                src={`${STORAGE_PATHS.PRODUCT.THUMBNAIL}/${item.imageUrl}`}
                width={80}
                height={80}
                alt=""
        ...
        <section className={styles.total_card}>
          ...
          <div className={styles.total_value}>
            <span>{formatWon(totals.total)}원</span>
          </div>
        </section>

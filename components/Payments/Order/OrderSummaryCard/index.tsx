        ...
        <div className={styles.card}>
            ...
            {orderItems.map((item: OrderItem, idx: number) => (
              <div className={styles.product_box} key={`${item.productId}-${idx}`}>
                <Image
                  ...
                  alt={item.korName ?? item.engName ?? `product-${item.productId}`}
                  className={styles.image}
                  width={80}
                  height={80}
                />
                <div className={styles.details}>
                  <div className={styles.name}>{item.engName ?? ''}</div>
                  <div className={styles.subtitle}>{item.korName ?? ''}</div>
                  <div className={styles.model}>#{item.productId}</div>
                  <div className={styles.meta}>
                    <span className={styles.size}>{item.optionValue ?? ''}</span>
                ...
                <div className={styles.price_wrap}>
                  <div className={styles.price}>{(item.price * item.quantity).toLocaleString()}원</div>
                  <button
          ...
          <div className={styles.coupon_row2}>
            ...
            {selectedCouponName && onClearCoupon ? (
              ...
              <button className={styles.cancel_btn} onClick={onOpenCouponModal}>
                쿠폰 선택
              </button>
            )}
          </div>
          ...
          <p className={styles.notice}>
            상품은 개인 또는 입점 사업자가 판매하며
            <br />각 거래 조건 등은 판매자 정보를 통해 확인해주시기 바랍니다.
          </p>
        </div>
      </section>
